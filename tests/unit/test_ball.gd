...
func test_increase_speed_adds_increment() -> void:
	...
	assert_almost_eq(
		...
		GameRules.BALL_SPEED_MIN + GameRules.BALL_SPEED_INCREMENT,
		0.01
	)


func test_increase_speed_clamps_at_max() -> void:
	_ball.linear_velocity = Vector2(GameRules.BALL_SPEED_MAX, 0.0)
	_ball.increase_speed()
	assert_almost_eq(_ball.linear_velocity.length(), GameRules.BALL_SPEED_MAX, 0.01)


func test_increase_speed_does_not_exceed_max_near_ceiling() -> void:
	_ball.linear_velocity = Vector2(GameRules.BALL_SPEED_MAX - 1.0, 0.0)
	_ball.increase_speed()
	assert_almost_eq(_ball.linear_velocity.length(), GameRules.BALL_SPEED_MAX, 0.01)
